...
(`app/models/inventory*.py`) + миграция 0009. НЕТ репозитория, сервиса выдачи/
экипировки и хендлеров бота — ни один рантайм-код не читает и не пишет эти
таблицы. Сайт читает только агрегаты (`COUNT/SUM`) для статистики профиля.
Магазин, UI и конкретные предметы — НЕ делались.
