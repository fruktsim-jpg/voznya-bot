...
_JOKE_REQUEST_PAT = (
    "расскажи анекдот", "дай анекдот", "анекдот", "пошути", "расскажи шутку",
    "дай шутку", "шутку", "смешно скажи", "смешное", "зарофли", "придумай прикол",
    "придумай шутку", "придумай анекдот", "рассмеши", "joke",
)

_FUN_FACT_PAT = (
    "забавный факт", "интересный факт", "факт", "расскажи что-нибудь",
    "расскажи чтонить", "удиви", "байку", "байк", "историю из чата",
)
...
_MODES: dict[str, str] = {
    ...
    "help": (
        ...
        "конкретно: 2-5 шагов, пример, или прямой вывод. НЕ отбрехивайся, НЕ "
        "уходи в чистый рофл, не делай вид, что ты просто комментатор. Дай "
        "рабочую подсказку, потом можешь подколоть. Если данных не хватает — "
        "скажи, что именно неизвестно, и предложи лучший следующий шаг."
    ),
    "question": (
        ...
        "вопрос = слив. Если точного факта нет в контексте, не выдумывай: дай "
        "осторожную версию, объясни, чего не хватает, и как это проверить."
    ),
    "threat_joke": (
    ...
    "joke": (
        "Человек прямо просит шутку/анекдот. НЕ выдавай дежурный панч про "
        "проигранные ешки, дуэли, плохой КД или казино, если он сам этого не "
        "попросил. Сочини свежую мини-шутку из текущего контекста, памяти, "
        "характера людей или абсурда мира VOZNYA. Формат: 1 короткий сетап + "
        "1 панчлайн, можно с локальным именем/кличкой, но без однотипной "
        "экономической жвачки. Лучше странно и метко, чем токсично по шаблону."
    ),
    "fun_fact": (
        "Человек просит не гайд и не роаст, а забавный факт/байку/что-нибудь из "
        "памяти. Дай ОДИН конкретный факт или сцену из контекста/архива/летописи, "
        "а не универсальный подкол. Если факта нет — честно преврати это в "
        "короткую байку по текущему чату, но не уходи в ешки/дуэли по привычке."
    ),
    "aggression": (
...
_GLOBAL_DIALOGUE_RULES = (
    "\n\nГлобально для этого ответа:\n"
    "- НЕ отвечай одной и той же манерой каждый раз; выбери свежий заход.\n"
    "- Если это вопрос или просьба — сначала полезная суть, потом персона/рофл.\n"
    "- Если это диалог, продолжай диалог: задай один уместный встречный вопрос "
    "или дай человеку зацепку ответить.\n"
    "- Не своди всё к ешкам, старым фактам или одному любимому подколу, если "
    "человек не спрашивал об этом.\n"
    "- Лучше 1-4 живые фразы, чем простыня или случайный набор токсичных панчей."
)
...
def classify_response_mode(text: str, *, addressed: bool = True) -> ResponseMode:
    ...
    if _has(low, _JOKE_REQUEST_PAT):
        return ResponseMode("joke", _MODES["joke"])
    if _has(low, _FUN_FACT_PAT):
        return ResponseMode("fun_fact", _MODES["fun_fact"])
...
def mode_directive(text: str, *, addressed: bool = True) -> tuple[str, str]:
    ...
    block = (
        f"# КАК ОТВЕТИТЬ ИМЕННО СЕЙЧАС (режим: {mode.name})\n"
        f"{mode.directive}{_GLOBAL_DIALOGUE_RULES}"
    )
