...
async def _safe_json(resp: Any) -> tuple[Any, str]:
    """Парсит тело ответа без падения на не-JSON.

    INCIDENT 2026-06-18 part 4: gateway (wellflow и пр.) при сбое отдаёт
    пустое тело / HTML 502 / plain-text rate-limit. Старый код звал
    ``resp.json(content_type=None)`` ДО проверки статуса, и stdlib
    ``json.JSONDecodeError`` (это ``ValueError``, НЕ ``aiohttp.ClientError``)
    пробивал внешний ``except`` в :func:`chat`, выскакивал из
    ``service.generate`` и валил весь хэндлер aiogram с молчанием для
    пользователя. Возвращаем ``(parsed_or_None, raw_snippet)`` — вызывающий
    код сам решает, бросить :class:`LlmError` с понятным сообщением.
    """
    try:
        raw = await resp.text()
    except Exception as exc:  # noqa: BLE001 — соединение могло обвалиться
        return None, f"<body read failed: {exc}>"
...
async def _openai_chat(
    ...
    async with http.post(url, json=payload, headers=headers) as resp:
        ...
        if data is None:
            ...
            raise LlmError(f"non-json body from provider (status {resp.status}): {raw[:200]!r}")
        try:
