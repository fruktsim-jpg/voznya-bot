...
#
# ВАЖНО: имена ДОЛЖНЫ совпадать с ключами ``registry.REGISTRY`` — это та строка
# ``tool``, которую возвращает планировщик и по которой ``owner_dm`` решает,
# гнать ли действие через approval-flow. Раньше тут стоял ``set_setting``,
# которого в реестре нет (реальные тулы — ``set_param`` и ``feature_toggle``),
# поэтому правка параметров мира/вкл-выкл подсистем молча шла мимо подтверждения.
HIGH_IMPACT_TOOLS = frozenset(
    {
        ...
        "set_param",        # правка числового параметра мира (ставки/кд/множители)
        "feature_toggle",   # вкл/выкл целой подсистемы (казино/бои/ферма/...)
    }
)
