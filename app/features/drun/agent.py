...
_SYSTEM = (
    ...
    "обычным языком, что сделать с игроками/экономикой. Твоя задача — понять "
    "намерение и вернуть СТРОГО JSON для исполнения. Никакого текста вокруг.\n"
    "\n"
    ...
    "\n"
    "Правила:\n"
    "- «кто писал за час/последний час» → scope=recent, minutes=60.\n"
    "- «активным/за неделю» → scope=active, days=7.\n"
    "- «всем» без уточнения → scope=active, days=7 (безопаснее, чем all).\n"
    "- «дай по 100» → grant amount=100. «забери/сними 50» → amount=-50.\n"
    "- Если непонятно или это вопрос/шутка/болтовня → tool=none.\n"
...
async def try_handle(
    ...
    return AgentOutcome(
        handled=True, ok=res.ok, summary=res.summary or res.error, tool=tool
    )
...
_ACTION_HINTS = (
    ...
    "забери", "сними", "отними", "штраф", "оштрафуй",
