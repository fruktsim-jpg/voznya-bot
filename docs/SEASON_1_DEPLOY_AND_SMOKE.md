...
**бэкап → smoke-test на staging → бэкап → деплой(=вайп) → старт сезона**.

Вайп (0034) НЕОБРАТИМ.

> 🔴 КРИТИЧНО — АВТО-МИГРАЦИЯ ПРИ СТАРТЕ КОНТЕЙНЕРА.
> `Dockerfile` запускает `alembic upgrade head` при каждом старте бота. Цепочка
> миграций линейная: `0033 → 0034(вайп) → 0035`, и 0035 (сезонный кейс) имеет
> родителем 0034. Это значит:
> 1. `docker compose up -d --build` НА ПРОДЕ АВТОМАТИЧЕСКИ ВЫПОЛНИТ ВАЙП (0034).
>    Отдельно «применить только 0033» в обычном старте контейнера нельзя —
>    апгрейд всегда идёт до head.
> 2. Поэтому smoke-test делаем НА STAGING-копии, а на проде пересборку бота
>    запускаем ТОЛЬКО в окне обслуживания, осознанно, сразу после свежего
>    бэкапа (раздел 3). До этого момента прод НЕ пересобирать.
...
## 1. Smoke-test НА STAGING (обратимая часть)

Выполнять на STAGING-копии прод-БД (восстановленной из свежего бэкапа). На
проде этот раздел НЕ выполняем — там единый деплой в окне обслуживания (§3).

```bash
git pull

# Бэкап staging ДО изменений.
pg_dump "$DATABASE_URL" > backup_staging_$(date +%F).sql

# На staging миграции можно применять по одной (для проверки 0033 отдельно).
# Схема сезона (обратимо — есть downgrade).
alembic upgrade 0033_season_system

# Прогон логических тестов бота.
pytest -q tests/test_season_logic.py tests/test_achievements_catalog.py

# Сборка и перезапуск сайта/бота на staging.
# ВНИМАНИЕ: контейнер при старте делает `alembic upgrade head`, т.е. на staging
# это применит 0034(вайп)+0035 автоматически — что и нужно для smoke-test.
docker compose up -d --build
```

Дальше прогнать раздел 2 целиком. Только при ЗЕЛЁНОМ smoke-test переходим к §3.
