...
Статус: бэкенд-ядро + бот реализованы (этапы 1–6); веб/Mini App — проектирование.
Этот документ описывает архитектуру и порядок работ.

---

# ПЕРЕСМОТР АРХИТЕКТУРЫ (web/Mini App-first) — обязателен к прочтению

Приоритет интерфейсов изменён: **1) Сайт, 2) Telegram Mini App, 3) Бот**. UX
кейсов (витрина, содержимое, шансы, история, красивые открытия, коллекции,
Gifts, Stars) — в вебе/Mini App. Бот — тонкий клиент. Источник истины прежний:
PostgreSQL + бот как игровое ядро (единственный писатель gameplay-таблиц).

## Главный инвариант: один backend flow для всех интерфейсов

Открытие кейса реализовано ОДИН раз на Python — `open_case()` (→ `grant_reward()`).
Эта функция уже НЕ зависит от aiogram: принимает `session` + `user_id`, ничего не
знает про Telegram. Поэтому переписывать открытие на TS нельзя и не нужно —
сайт и Mini App вызывают тот же Python-flow по HTTP. Отдельных реализаций для
разных интерфейсов не существует — это обеспечено архитектурно (единственная
точка входа — open-endpoint).

```
Сайт (Next.js)  ─┐
Mini App (web)  ─┼─► POST /api/cases/{code}/open ─► open_case() ─► grant_reward()
Бот (aiogram)   ─┘            (Python API)            (одна реализация)   │
                                                                          ▼
                              PostgreSQL: case_openings + inventory_history + transactions
```

Бот вызывает `open_case()` in-process (как сейчас); сайт и Mini App — по HTTP.
Next.js-роуты НЕ содержат логики открытия — только проксируют/рендерят.

## Ответы на ключевые вопросы

1. **Через сайт** — весь UX: витрина, содержимое, шансы, история открытий,
   анимация открытия, коллекции. Кнопка «Открыть» → POST на open-endpoint →
   рендер результата. Сайт пишет gameplay НЕ напрямую, а только через этот
   endpoint ядра (инвариант AGENTS.md сохранён: единственный писатель — ядро бота).
2. **Через Mini App** — то же самое, тот же endpoint и те же React-компоненты.
   Отличие только в аутентификации (Telegram `initData` HMAC вместо сессии сайта)
   и в нативных платежах (Stars/Gifts — позже).
3. **В боте** — текстовый быстрый путь (`/кейсы`, `/открыть`) + кнопка, ведущая
   в Mini App для красивого открытия. Бот остаётся игровым ядром (процесс,
   владеющий БД), но как интерфейс — третичен.
4. **API ядра** (новый HTTP-слой на Python, напр. FastAPI в том же
   процессе/контейнере, что и бот — общий доступ к моделям и `open_case`):
   - `GET  /api/cases` — витрина (активные кейсы);
   - `GET  /api/cases/{code}` — содержимое + шансы (из `weight`);
   - `POST /api/cases/{code}/open` — ЕДИНСТВЕННЫЙ путь открытия; внутри
     `open_case()`; игрок берётся из auth, не из тела;
   - `GET  /api/cases/openings?user=` — история (из `case_openings`);
   - аутентификация: сессия сайта (OIDC-привязка, уже есть) ИЛИ Telegram
     `initData` для Mini App → оба резолвятся в `user_id`.
5. **Без дублирования**: логика открытия только в Python (`open_case`/
   `grant_reward`). TS-кода открытия нет вообще — TS только рендерит. Любой
   интерфейс физически открывает кейс лишь через open-endpoint.

## Влияние на этапы 7–8 (СМЕНА НАПРАВЛЕНИЯ — ждёт согласования)

Было: «сайт = read-only витрина на Next.js». Стало: «Python API ядра +
веб-клиенты (сайт/Mini App) поверх него». Этапы 7–8 переопределяются:
- 7) HTTP-слой ядра (FastAPI): cases-endpoints + auth-мост (сессия сайта /
  Mini App `initData`) + admin CRUD endpoints поверх тех же моделей;
- 8) веб-клиенты: витрина/содержимое/шансы/история/анимация на Next.js,
  переиспользуемые в Mini App; кнопка из бота в Mini App.

Расширяемость под Gifts/Stars/Cosmetics/Collections/Seasonal Pass уже заложена
в схеме (см. ниже): `reward_kind` (задел tg_gift/stars), `inventory_instances`,
`open_cost_kind='stars'`, `collection_code`/`series_total`/`season_code`.
