...
Статус (2026-06-06, проверено аудитом): **FOUNDATION ONLY.** Есть 3 модели
(`shop_category`, `shop_offer`, `purchase_history`) + миграция 0010. НЕТ логики
покупки, списания ешек, витрины, репозитория и API — ни один рантайм-код не
использует эти таблицы. Зависит от инвентаря, который тоже foundation-only.
UI магазина, подарки и маркетплейс — НЕ делались.
